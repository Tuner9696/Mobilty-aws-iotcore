...
class RosToMqttBridge(Bridge):
    ...
    def _publish(self, msg):
        cleaned_msg = self.cleanNullTerms(extract_values(msg))
        print('ROS to MQTT Published message: ')
        print(cleaned_msg)
        payload = bytearray(self._serialize(cleaned_msg))
        ...
        for k, v in d.items():
            if isinstance(v, dict):
                ...
                if len(nested.keys()) > 0:
                    clean[k] = nested
            elif v is not None and (v or type(v) == type(True)):
                clean[k] = v
        return clean
    ...
    def _callback_mqtt(self, client, userdata, mqtt_msg):
        ...
        if self._interval is None or now - self._last_published >= self._interval:
            try:
                ...
                self._last_published = now
                print("MQTT to ROS Received message: ")
                print(ros_msg)
